...
;; Generic types
;;
(type radio_frame               vector u8 107)
...
(type res_poll union
    (fields
        (empty none)
        (empty needs_reset)
        (field frame            radio_frame)))

;; Send data from application to platform
(type cmd_send union
    (fields
        (field req              radio_frame)
        (field res              res_send)))
...
(type res_send union
    (fields
        (empty ok)
        (empty err_not_connected)
        (empty err_dropped)
        (empty err_nack)
        (empty err_channel_access)))
...
(type res_connect enumeration
     (values
        ...
        ok
        ;; Can only connect in autonomous mode
        err_not_autonomous
...
;; Only used for connecting (where we need quickconnect data + time)
;; When shutting down Atom, all we only give the application the
;; quickconnect data and it will keep track of time while Atom is
;; asleep
(type quick_connect record
    (fields
        ;; Unix time
        (field time             u32)
        ;; Connection metadata
